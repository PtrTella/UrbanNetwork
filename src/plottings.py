...
def finalize_map(fig, ax, title):
    ...
    try:
        ctx.add_basemap(ax, crs="EPSG:4326", source=ctx.providers.CartoDB.Positron, alpha=0.85)
...
def draw_transit_network(ax, G, pos, tram_color, bus_color="#bdc3c7"):
    ...
    # Bus edges
    bus_edges = [(u, v) for u, v, d in G.edges(data=True) if d.get("type") == "bus" and u in pos and v in pos]
    nx.draw_networkx_edges(G, pos, edgelist=bus_edges, ax=ax, edge_color=bus_color, width=0.8, alpha=0.3)
    
    # Tram edges
    tram_edges = [(u, v) for u, v, d in G.edges(data=True) if d.get("type") == "tram" and u in pos and v in pos]
    nx.draw_networkx_edges(G, pos, edgelist=tram_edges, ax=ax, edge_color=tram_color, width=2.5, alpha=0.9)
    
    # Tram nodes (azure/light blue)
    tram_nodes = [n for n, d in G.nodes(data=True) if d.get("type") in ["tram", "intersezione_bus_tram"] and n in pos]
    if tram_nodes:
        nx.draw_networkx_nodes(G, pos, nodelist=tram_nodes, ax=ax, node_color="#3498db", node_size=30, edgecolors="white", linewidths=0.5, alpha=0.9)
...
def plot_resilience_curves():
    ...
    # Subplot 1: LCC Size
    ax1.plot(x, df["Random_LCC_Size"], label="Random Failure", color=c_rand, lw=2.5, ls="--")
    ax1.plot(x, df["Targeted_LCC_Size"], label="Targeted Attack (Hubs)", color=c_targ, lw=2.5)
    ax1.plot(x, df["Accidents_LCC_Size"], label="Accident Risk", color=c_acc, lw=2.5, marker="o", ms=4)
    apply_plot_style(ax1, "Topological Resilience (LCC Fragmentation)", "Removed Nodes (%)", "Giant Component Size (LCC)")
    ax1.set_ylim([0, 1.05])
    ax1.set_xlim([0, 50])
    ax1.legend()

    # Subplot 2: Efficiency
    ax2.plot(x, df["Random_Efficiency"], label="Random Failure", color=c_rand, lw=2.5, ls="--")
    ax2.plot(x, df["Targeted_Efficiency"], label="Targeted Attack (Hubs)", color=c_targ, lw=2.5)
    ax2.plot(x, df["Accidents_Efficiency"], label="Accident Risk", color=c_acc, lw=2.5, marker="o", ms=4)
    apply_plot_style(ax2, "Dynamic Resilience (Travel Times)", "Removed Nodes (%)", "Residual Global Efficiency")
    ax2.set_ylim([0, 1.05])
    ax2.set_xlim([0, 50])
    ax2.legend()
...
def plot_centrality_analysis(df_cent_bus=None, df_cent_tram=None):
    ...
    # Bus Only (Blue)
    sns.regplot(
        data=df_cent_bus, x="Degree", y="Betweenness Centrality", x_jitter=0.15,
    ...
    # Planned Tram (Green)
    sns.regplot(
        data=df_cent_tram, x="Degree", y="Betweenness Centrality", x_jitter=0.15,
    ...
    sns.scatterplot(
        data=top_5_bus, x="Degree", y="Betweenness Centrality",
        color="#e74c3c", s=120, marker="o", edgecolor="black", linewidth=1.5, ax=ax,
        label="Bus Only Bottlenecks",
    )

    top_5_tram = df_cent_tram.nlargest(5, "Betweenness Centrality")
    sns.scatterplot(
        data=top_5_tram, x="Degree", y="Betweenness Centrality",
        color="#f39c12", s=120, marker="s", edgecolor="black", linewidth=1.5, ax=ax,
        label="Planned Tram Bottlenecks",
    )

    # Labels for top bottlenecks
    for _, row in top_5_bus.iterrows():
        ax.text(row["Degree"] + 0.1, row["Betweenness Centrality"], f"{row['Station_Name']} (B)",
                fontsize=8, fontweight="bold", color="#2c3e50")
        
    for _, row in top_5_tram.iterrows():
        offset_y = -0.005 if row['Station_Name'] in top_5_bus['Station_Name'].values else 0.0
        ax.text(row["Degree"] - 0.5, row["Betweenness Centrality"] + offset_y, f"{row['Station_Name']} (T)",
                fontsize=8, fontweight="bold", color="#27ae60")

    apply_plot_style(ax, "Node Degree vs Betweenness Centrality Comparison", "Node Degree", "Betweenness Centrality (Weighted)")
    ax.legend()
    plt.tight_layout()
    ...
    sns.histplot(
        ...
        x="Betweenness Centrality", kde=True, color="#3498db", bins=30, ax=ax,
        stat="density", alpha=0.4, label="Bus Only"
    )
    sns.histplot(
        data=df_cent_tram[df_cent_tram["Betweenness Centrality"] > 0],
        x="Betweenness Centrality", kde=True, color="#2ecc71", bins=30, ax=ax,
        stat="density", alpha=0.4, label="Planned Tram"
    )
    apply_plot_style(ax, "Betweenness Centrality Distribution (Values > 0)", "Betweenness Centrality", "Density")
    ax.legend()
    plt.tight_layout()
...
def plot_static_network(G):
    ...
    def get_bus_color(route_id):
        colors = ["#9b59b6", "#1abc9c", "#f1c40f", "#34495e", "#e67e22", "#16a085", "#8e44ad", "#d35400"]
        return colors[hash(str(route_id)) % len(colors)]

    for route_id, edgelist in bus_edges_by_route.items():
        nx.draw_networkx_edges(G, pos, edgelist=edgelist, ax=ax, edge_color=get_bus_color(route_id), width=0.8, alpha=0.35)
    ...
    for route_id, edgelist in tram_edges_by_route.items():
        color = "#e74c3c" if any(k in route_id for k in ["RED", "ROSSA"]) else "#27ae60"
        nx.draw_networkx_edges(G, pos, edgelist=edgelist, ax=ax, edge_color=color, width=3.0, alpha=0.85)

    # 3. Nodes
    bus_nodes = [n for n, d in G.nodes(data=True) if d.get("type") == "bus" and n in pos]
    nx.draw_networkx_nodes(G, pos, nodelist=bus_nodes, ax=ax, node_color="#7f8c8d", node_size=10, alpha=0.5)

    # All tram nodes (both 'tram' and 'intersezione_bus_tram') painted Azure
    tram_nodes = [n for n, d in G.nodes(data=True) if d.get("type") in ["tram", "intersezione_bus_tram"] and n in pos]
    if tram_nodes:
        nx.draw_networkx_nodes(G, pos, nodelist=tram_nodes, ax=ax, node_color="#3498db", node_size=35, edgecolors="white", linewidths=0.5, alpha=0.95)
...
def plot_optimized_layout(G_planned, G_opt):
    ...
    pos_planned = {n: (d["lon"], d["lat"]) for n, d in G_planned.nodes(data=True) if "lon" in d and "lat" in d}
    pos_opt = {n: (d["lon"], d["lat"]) for n, d in G_opt.nodes(data=True) if "lon" in d and "lat" in d}
    ...
    plt.suptitle("Bologna: Planned vs Optimal Layout Comparison (25 km Budget)", fontsize=16, fontweight="bold", y=0.98)
    plt.tight_layout()
...
def plot_demographic_pressure_maps():
    ...
    fig, ax = plt.subplots(figsize=(12, 10))
    sns.kdeplot(
        data=df_tram, x="Longitude", y="Latitude", weights="Population_Served",
        fill=True, cmap="Oranges", alpha=0.55, levels=100, thresh=0.05, ax=ax
    )
...
def plot_communities_map():
    ...
    fig, ax = plt.subplots(figsize=(12, 10))
    sns.scatterplot(data=df_comm, x="Longitude", y="Latitude", hue="Community_ID", palette="tab20", s=60, alpha=0.8, edgecolor="white", legend=False, ax=ax)
...
def plot_bottlenecks_map():
    ...
    sns.scatterplot(
        data=df_bus, x="Longitude", y="Latitude", size="Betweenness Centrality",
        color="#7f8c8d", sizes=(20, 500), alpha=0.5, legend=False, ax=ax
    )
    # Plot Planned Tram in grayscale
    sns.scatterplot(
        data=df_tram, x="Longitude", y="Latitude", size="Betweenness Centrality",
        color="#95a5a6", sizes=(20, 500), alpha=0.5, legend=False, ax=ax
    )
    ...
    for _, row in top_5_bus.iterrows():
        ax.text(
            row["Longitude"] + 0.0008, row["Latitude"] + 0.0008, f"{row['Station_Name']} (Bus Only)",
            fontsize=8, fontweight="bold", color="white",
            bbox=dict(facecolor='#2980b9', alpha=0.85, edgecolor='none', boxstyle='round,pad=0.2')
        )
        
    top_5_tram = df_tram.nlargest(5, "Betweenness Centrality")
    for _, row in top_5_tram.iterrows():
        offset_y = -0.0008 if row['Station_Name'] in top_5_bus['Station_Name'].values else 0.0008
        ax.text(
            row["Longitude"] - 0.0008, row["Latitude"] + offset_y, f"{row['Station_Name']} (Planned Tram)",
            fontsize=8, fontweight="bold", color="white",
            bbox=dict(facecolor='#27ae60', alpha=0.85, edgecolor='none', boxstyle='round,pad=0.2')
        )
        
    finalize_map(fig, ax, "Act 2: Physical Bottlenecks Comparison (L-Space Betweenness Centrality)")
    plt.tight_layout()
...
def plot_comparative_resilience():
    ...
    ax.plot(fractions * 100, bus_eff_drops, label="Bus Only", color="#e74c3c", lw=2.5, marker="o")
    ax.plot(fractions * 100, fused_eff_drops, label="Planned Tram", color="#2ecc71", lw=2.5, marker="s")
    
    apply_plot_style(ax, "Act 3: Resilience Mitigation (L-Space Target Attack)", "Removed Nodes (%)", "Residual Global Efficiency (%)")
    ax.set_ylim([0, 1.05])
    ax.set_xlim([0, 40])
    ax.legend()
...
def plot_tram_impact_map():
    ...
    if hub_nodes:
        nx.draw_networkx_nodes(G_fused, pos, nodelist=hub_nodes, ax=ax, node_color="#e74c3c", node_size=150, edgecolors="white", linewidths=1.5)
        for h in hub_nodes:
            ax.text(pos[h][0], pos[h][1] + 0.0012, "HUB", color="white", fontsize=10, fontweight="bold", ha="center",
                    bbox=dict(facecolor='#e74c3c', alpha=0.85, edgecolor='none', boxstyle='round,pad=0.25'))
...
def plot_forced_injection_shock(injection_steps, bus_times, tram_times, futuro_times, opt_times=None):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(injection_steps, bus_mins, label="Bus Only", color="#e74c3c", lw=2.5, marker="o")
    ax.plot(injection_steps, tram_mins, label="Planned Tram", color="#2ecc71", lw=2.5, marker="s")
    ax.plot(injection_steps, futuro_mins, label="Circular Tram", color="#3498db", lw=2.5, marker="^")
    
    if opt_times is not None:
        opt_mins = [t / 60.0 for t in opt_times]
        ax.plot(injection_steps, opt_mins, label="Optimal Tram", color="#9b59b6", lw=2.5, marker="D")
    ...
    apply_plot_style(ax, "Act 4: Forced Passenger Injection (Stazione/Autostazione Shock)", "Injected Commuters", "Average Travel Time (Minutes)")
