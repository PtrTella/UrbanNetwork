...
def compute_small_worldness(G, er_runs=50):
    ...
    for _ in range(er_runs):
        ...
        if nx.is_connected(G_rand):
            ...
            if len(r_comps[0]) > 1:
                rand_L_list.append(nx.average_shortest_path_length(G_rand.subgraph(r_comps[0])))
                rand_C_list.append(nx.average_clustering(G_rand))
    ...
    L_lat = nx.average_shortest_path_length(G_lat) if nx.is_connected(G_lat) else float('inf')
