...
\author{Pietro Tellarini, Artificial Intelligence, ID number (matricola) \\ Kawsar Farooq, Artificial Intelligence, ID number (matricola)}
\date{\today}
...
\subsection{Core-Periphery Structure: k-Core Decomposition}
To characterize the network's global structural organization beyond individual centralities, we apply a $k$-core decomposition. The $k$-core of a graph is the maximal subgraph in which all vertices have a degree of at least $k$. It is a standard method to study core-periphery structures in complex networks, identifying which nodes form a dense, cohesive nucleus ($k$-core) and which belong to peripheral branches ($k$-shell).

Our analysis of the London Underground L-space graph reveals a maximum coreness value of $k_{max} = 2$. The network is partitioned into two distinct shells:
\begin{itemize}
    \item \textbf{Innermost Core ($2$-core)}: Consists of $166$ stations. This core encompasses all major central London interchange hubs (e.g., Baker Street, Bank, Oxford Circus, King's Cross St. Pancras) and intermediate stations along the overlapping central lines. This forms a highly integrated, loop-rich structural core that can tolerate local path disconnections due to routing redundancies.
    \item \textbf{Periphery ($1$-core shell)}: Consists of $136$ stations. This shell is composed of peripheral branch lines and terminal stations (e.g., Amersham, Epping, Upminster, Heathrow terminals). Stations in this peripheral shell have a degree of 1, meaning they are connected via a single link to the rest of the network. A disruption at these nodes or their adjacent track segments instantly isolates the respective terminals, reflecting a classic tree-like peripheral architecture.
\end{itemize}
This $k$-core structure highlights that while the center of the London Underground is topologically redundant, the outer branches are highly vulnerable branch endpoints with zero redundancy.

\subsection{Degree Distribution and Scale-Free Behavior}
Many complex networks, such as social networks or the Internet, are characterized by a scale-free degree distribution that follows a power-law $P(k) \sim k^{-\gamma}$. Scale-free networks are dominated by a tiny fraction of massive hubs with hundreds or thousands of connections.

However, in spatial infrastructure networks, the physical layout is bounded by geographical constraints: tracks and platforms must occupy real physical space, imposing a strict limit on the number of connections a station can have. To evaluate this behavior in the London Underground, we plot the node degree distribution (Fig.~\ref{fig:degree_dist}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{figures/london_degree_dist.png}
\caption{Node degree distribution of the London Underground network. Node degrees are strictly bounded ($1 \le k \le 7$), showing a sharp exponential decay rather than scale-free scale-freedom.}
\label{fig:degree_dist}
\end{figure}

The empirical degree distribution is highly bounded, ranging strictly from $k = 1$ to a maximum of $k_{max} = 7$ (achieved only by Baker Street and King's Cross St. Pancras). The majority of stations ($166$ nodes) have a degree of $k=2$, representing intermediate stops along single lines. The frequency of nodes decays rapidly as degree increases beyond 2.

We can model this mathematically using an exponential distribution:
\begin{equation}
    P(k) \sim e^{-k/\lambda}
\end{equation}
The sharp truncation at $k=7$ confirms that the Tube is not a scale-free network. This planarity constraint limits the maximum degree of any single node, preventing the emergence of super-hubs. Consequently, the network's resilience profile is dominated by a moderately distributed set of bottlenecks rather than a single super-hub.
