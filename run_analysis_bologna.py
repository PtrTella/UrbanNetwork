...
def main():
    ...
    print(
        f"  - Bus-Only P-Space: {G_pspace_bus.number_of_nodes()} stops, {G_pspace_bus.number_of_edges()} cognitive edges."
    )
    print(
        f"  - Planned Tram P-Space: {G_pspace_fused.number_of_nodes()} stops, {G_pspace_fused.number_of_edges()} cognitive edges."
    )
    ...
    print(
        f"  - Density: L-Space (bus) = {dens_lspace:.5f} | P-Space (bus) = {dens_pspace:.4f}"
    )
    print(
        f"  - P-Space k-core (bus): {len(set(core_pspace.values()))} shells, innermost k = {k_max} with {inner_core_size} stops"
    )
    ...
    print("-" * 110)
    print(
        f"{'Rank':<5} | {'Bus Only (Baseline) Station':<35} | {'Score':<10} | {'Planned Tram (TPER) Station':<35} | {'Score':<10}"
    )
    print("-" * 110)
    ...
    for idx in range(5):
        ...
        print(
            f"{idx + 1:<5} | {bus_row['Station_Name']:<35} | {bus_row['Betweenness Centrality']:<10.5f} | {tram_row['Station_Name']:<35} | {tram_row['Betweenness Centrality']:<10.5f}"
        )
    print("-" * 110)
    ...
    print(
        f"{'Avg Travel Time L (sec)':<40} | {macro_bus['L']:<18.2f} | {macro_fused['L']:<18.2f}"
    )
    print(
        f"{'Clustering Coeff (C)':<40} | {macro_bus['C']:<18.4f} | {macro_fused['C']:<18.4f}"
    )
    print(
        f"{'Global Efficiency E_glob (s^-1)':<40} | {macro_bus['E_glob']:<18.6f} | {macro_fused['E_glob']:<18.6f}"
    )
    print(
        f"{'Small-World Sigma (sigma)':<40} | {macro_bus['Sigma']:<18.2f} | {macro_fused['Sigma']:<18.2f}"
    )
    print(
        f"{'Small-World Omega (omega)':<40} | {macro_bus['Omega']:<18.2f} | {macro_fused['Omega']:<18.2f}"
    )
    print("-" * 80)
    ...
    df_macro = pd.DataFrame([
        {
            ...
            "L": macro_bus["L"],
            "C": macro_bus["C"],
            "E_glob": macro_bus["E_glob"],
            "Sigma": macro_bus["Sigma"],
            "Omega": macro_bus["Omega"],
        },
        {
            "Scenario": "Planned Tram",
            "N": macro_fused["N"],
            "M": macro_fused["M"],
            "L": macro_fused["L"],
            "C": macro_fused["C"],
            "E_glob": macro_fused["E_glob"],
            "Sigma": macro_fused["Sigma"],
            "Omega": macro_fused["Omega"],
        },
    ])
    ...
    graphs_dict = {"Bus Only": G_bus, "Planned Tram": G_fused}
    ...
    (
        ...
        targ_eff,
        acc_eff,
    ) = run_resilience_simulation(G_bus, random_runs=20)
    ...
    print("\n" + "=" * 110)
    print(
        "ACT 4: FUTURE SCENARIOS COMPARATIVE PERFORMANCE & SHOCK INJECTION".center(110)
    )
    print("=" * 110)
    ...
    l_circular = nx.average_shortest_path_length(
        G_futuro
        if nx.is_connected(G_futuro)
        else G_futuro.subgraph(max(nx.connected_components(G_futuro), key=len)),
        weight="weight",
    )
    ...
    if G_opt:
        l_opt = nx.average_shortest_path_length(
            ...
            else G_opt.subgraph(max(nx.connected_components(G_opt), key=len)),
            weight="weight",
        )
    ...
    print("-" * 95)
    print(
        f"{'Scenario':<28} | {'Avg Travel Time L (sec)':<25} | {'Global Efficiency E_glob (s^-1)':<32} | {'Tram Pop Served':<15}"
    )
    print("-" * 95)
    print(f"{'Bus Only':<28} | {l_bus:<25.2f} | {eff_bus:<32.6f} | {'N/D':<15}")
    print(
        f"{'Planned Tram':<28} | {l_planned:<25.2f} | {eff_planned:<32.6f} | {int(pop_planned):<15,}"
    )
    print(
        f"{'Circular Tram':<28} | {l_circular:<25.2f} | {eff_circular:<32.6f} | {int(pop_circular):<15,}"
    )
    if G_opt:
        print(
            f"{'Optimal Tram':<28} | {l_opt:<25.2f} | {eff_opt:<32.6f} | {int(pop_opt):<15,}"
        )
    print("-" * 95)
    ...
    df_scenarios = pd.DataFrame([
        ...
        },
        {
            ...
            "Tram_Pop_Served": int(pop_planned),
        },
        {
            "Scenario": "Circular Tram",
            "Avg_Travel_Time_Sec": l_circular,
            "Global_Efficiency": eff_circular,
            "Tram_Pop_Served": int(pop_circular),
        },
        {
            "Scenario": "Optimal Tram",
            "Avg_Travel_Time_Sec": l_opt,
            "Global_Efficiency": eff_opt,
            "Tram_Pop_Served": int(pop_opt),
        },
    ])
    ...
    print("-" * 95)
    print(
        f"{'Shock Population':<18} | {'Bus Only (min)':<16} | {'Planned Tram (min)':<18} | {'Circular Tram (min)':<19} | {'Optimal Tram (min)':<18}"
    )
    print("-" * 95)
    ...
    plot_tram_impact_map()
    plot_forced_injection_shock(
        injection_steps, bus_shock, tram_shock, circular_shock, opt_shock
    )
    plot_demographic_pressure_maps()
